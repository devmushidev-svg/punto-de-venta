...
import { isCreditSaleTerm, isImmediateSaleTerm, normalizeSaleTerms, resolveSalePaid, } from "./lib/saleTerms.js";
import { assertKitSaleStock, decrementStockForKitSale, replaceProductKitLines, } from "./lib/kit.js";
...
import { effectivePermissionList, parsePermissionsJson, PERMISSION_KEYS, serializePermissionsPayload, userHasPermission, } from "./lib/permissions.js";
import { applyTransferReceiveStock, applyTransferSendStock, assertStockForTransferSend, validateTransferLineProducts, } from "./lib/stockTransfer.js";
const PRODUCT_TYPES = ["PRODUCTO", "SERVICIO", "INSUMO", "KIT"];
function isValidProductType(pt) {
    return PRODUCT_TYPES.includes(pt);
}
...
app.post("/auth/login", async (c) => {
    ...
    if (!orgId) {
        ...
        if (!first)
            return c.json({ error: "No hay empresas registradas" }, 400);
        orgId = first.id;
    }
...
api.get("/auth/me", async (c) => {
    ...
    if (user.organizationId !== jwt.orgId)
        return c.json({ error: "No autorizado" }, 401);
...
api.post("/users", requireAdmin, async (c) => {
    ...
    if (body.permissionsJson !== undefined) {
        const raw = typeof body.permissionsJson === "string"
...
api.patch("/users/:id", requireAdmin, async (c) => {
    ...
    const bumpPermRev = (data.role !== undefined && data.role !== existing.role) ||
...
api.get("/products", async (c) => {
    ...
    const where = { organizationId: jwt.orgId };
    if (touch === "1") {
        where.active = true;
...
api.post("/products", requireAdmin, async (c) => {
    ...
    if (!sku || !body.name?.trim())
        return c.json({ error: "SKU y nombre requeridos" }, 400);
    ...
    try {
        ...
        if (msg === "KIT_COMPONENT_NOT_FOUND")
            return c.json({ error: "Componente no encontrado" }, 400);
        if (msg === "KIT_SELF_REF")
            return c.json({ error: "Un kit no puede incluirse a sí mismo" }, 400);
        if (msg === "KIT_DUP_COMPONENT")
            return c.json({ error: "No repita el mismo producto en el kit" }, 400);
...
api.patch("/products/:id", requireAdmin, async (c) => {
    ...
    if (body.active !== undefined)
        data.active = Boolean(body.active);
    if (body.esGranel !== undefined)
        data.esGranel = Boolean(body.esGranel);
    if (body.supplierId !== undefined)
        data.supplierId = body.supplierId === "" ? null : body.supplierId;
    ...
    const kitLinesBody = body.kitLines;
    try {
...
async function restoreStockForSaleLine(tx, line) {
    const prod = await tx.product.findUnique({ where: { id: line.productId } });
    if (!prod)
        throw new Error("PRODUCT_NOT_FOUND");
    if (prod.productType === "SERVICIO")
        return;
...
api.get("/suppliers", async (c) => {
    const jwt = c.get("jwt");
    return c.json(await prisma.supplier.findMany({
        where: { organizationId: jwt.orgId },
        orderBy: { name: "asc" },
        take: 2000,
    }));
...
api.post("/sales", async (c) => {
    ...
    if (!body.lines?.length)
        return c.json({ error: "Agregue líneas" }, 400);
    ...
    if (isCreditSaleTerm(terms)) {
        ...
        if (!cust)
            return c.json({ error: "Cliente no encontrado" }, 400);
    }
    try {
        const result = await prisma.$transaction(async (tx) => {
            ...
            for (const line of body.lines) {
                ...
                if (!product)
                    throw new Error("PRODUCT_NOT_FOUND");
                if (product.productType === "INSUMO")
                    throw new Error("INSUMO_NOT_SALEABLE");
        ...
        if (msg === "PRODUCT_NOT_FOUND")
            return c.json({ error: "Producto no encontrado" }, 400);
        if (msg === "INSUFFICIENT_STOCK")
            return c.json({ error: "Stock insuficiente" }, 400);
        if (msg === "KIT_EMPTY")
            return c.json({ error: "El kit no tiene componentes configurados" }, 400);
        if (msg === "KIT_BAD_COMPONENT")
            return c.json({ error: "Error en componentes del kit" }, 400);
        if (msg === "INSUMO_NOT_SALEABLE")
            return c.json({ error: "Los insumos no se venden en POS" }, 400);
        throw e;
    }
});
api.get("/sales", async (c) => {
    ...
    const immediateTerms = ["CONTADO", "TARJETA", "EFECTIVO"];
    const where = { organizationId: jwt.orgId };
    if (from || to) {
        where.saleDate = {};
        if (from)
...
api.patch("/sales/:id", requireAdmin, async (c) => {
    ...
    const sellerNamePatch = body.sellerName !== undefined
        ? typeof body.sellerName === "string"
            ? body.sellerName.trim() || null
            : null
        : undefined;
    try {
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (existing.receivableSurcharges.length > 0)
                throw new Error("SALE_HAS_SURCHARGES");
            ...
            await tx.sale.update({
                ...
                data: {
                    ...
                    terms,
                    notes: body.notes,
                    ...(sellerNamePatch !== undefined ? { sellerName: sellerNamePatch } : {}),
                    priceTier,
                    subtotal,
                    tax,
                    total,
...
api.get("/sales/:saleId/comprobante.pdf", async (c) => {
    ...
    const invoice = JSON.parse(settings.invoiceJson || "{}");
        ...
        const purchase = await prisma.$transaction(async (tx) => {
            ...
            for (const line of body.lines) {
                ...
                if (product.productType === "KIT")
                    throw new Error("KIT_NOT_PURCHASABLE");
...
api.get("/purchases", requireAnyPermission(PERMISSION_KEYS.PURCHASES_RECORD, PERMISSION_KEYS.PURCHASES_VIEW), async (c) => {
    const jwt = c.get("jwt");
    return c.json(await prisma.purchase.findMany({
        where: { organizationId: jwt.orgId },
        orderBy: { createdAt: "desc" },
        take: 200,
...
api.post("/cash-sessions/open", async (c) => {
    ...
    if (open)
        return c.json({ error: "Ya tiene caja abierta", session: open }, 400);
    const body = await c.req.json();
...
async function buildCashDiaryForSession(orgId, userId, session, end) {
    const sales = await prisma.sale.findMany({
        where: {
    ...
    const efectivoCajaSugerido = session.openingCash + efectivoVentasTotal + cobradoEfectivoCredito - gastosSesion;
    return {
        session: {
            id: session.id,
...
api.get("/reports/top-products", requirePermission(PERMISSION_KEYS.REPORTS_VIEW), async (c) => {
    ...
    const saleWhere = { organizationId: jwt.orgId };
    if (from || to) {
        saleWhere.saleDate = {};
        if (from)
            saleWhere.saleDate.gte = new Date(from);
        if (to) {
            const end = new Date(to);
            end.setHours(23, 59, 59, 999);
...
api.post("/accounts/receivable/:saleId/surcharge", requirePermission(PERMISSION_KEYS.ACCOUNTS_RECEIVABLE), async (c) => {
    ...
    if (!sale || !isCreditSaleTerm(sale.terms))
        return c.json({ error: "Venta no encontrada" }, 404);
...
api.post("/accounts/receivable/:saleId/pay", requirePermission(PERMISSION_KEYS.ACCOUNTS_RECEIVABLE), async (c) => {
    ...
    if (amount > balance)
        return c.json({ error: "Excede saldo" }, 400);
...
api.get("/quotes", async (c) => {
    const jwt = c.get("jwt");
    return c.json(await prisma.quote.findMany({
        where: { organizationId: jwt.orgId },
        orderBy: { createdAt: "desc" },
        take: 300,
...
api.post("/quotes", async (c) => {
    ...
    try {
        ...
        if (msg === "PRODUCT_NOT_FOUND")
            return c.json({ error: "Producto no encontrado" }, 400);
        if (msg === "INSUMO_NOT_SALEABLE")
            return c.json({ error: "Los insumos no se incluyen en cotizaciones" }, 400);
        throw e;
    }
...
api.patch("/quotes/:id", async (c) => {
    ...
    if (existing.status === "CONVERTIDA")
        return c.json({ error: "No se puede editar una cotización ya convertida" }, 400);
    let updated;
    try {
...
api.post("/quotes/:id/convert-to-sale", async (c) => {
    const jwt = c.get("jwt");
    const id = c.req.param("id");
    const convertBody = (await c.req.json().catch(() => ({})));
    ...
    if (quote.status === "CONVERTIDA")
        return c.json({ error: "Ya convertida" }, 400);
    const bodyCust = typeof convertBody.customerId === "string" && convertBody.customerId.trim()
        ? convertBody.customerId.trim()
        : null;
    const resolvedCustomerId = bodyCust ?? quote.customerId;
    if (isCreditSaleTerm(terms)) {
        if (!resolvedCustomerId) {
            return c.json({ error: "Las ventas a crédito requieren un cliente (asígnelo a la cotización o envíe customerId)" }, 400);
        }
        const cust = await prisma.customer.findFirst({
    ...
    if (!sale)
        return c.json({ error: "No se pudo convertir (stock o datos)" }, 400);
    return c.json(sale);
});
api.get("/orders", async (c) => {
    const jwt = c.get("jwt");
    return c.json(await prisma.supplierOrder.findMany({
        where: { organizationId: jwt.orgId },
        orderBy: { createdAt: "desc" },
        take: 300,
        include: { supplier: true, lines: { include: { product: true } } },
...
function mergeTransferLines(lines) {
    const m = new Map();
...
api.post("/stock-locations", requireAdmin, async (c) => {
    ...
    if (!code || !body.name?.trim())
        return c.json({ error: "Código y nombre requeridos" }, 400);
    try {
...
api.post("/stock-transfers", requirePermission(PERMISSION_KEYS.INVENTORY_TRANSFERS), async (c) => {
    ...
    const body = await c.req.json();
    if (!body.lines?.length)
        return c.json({ error: "Agregue líneas al traslado" }, 400);
    ...
    try {
        const t = await prisma.$transaction(async (tx) => {
            ...
            if (!fromLoc || !toLoc)
                throw new Error("TRANSFER_LOC_NOT_FOUND");
        ...
        const msg = e instanceof Error ? e.message : "";
        if (msg === "TRANSFER_LOC_NOT_FOUND")
            return c.json({ error: "Ubicación no válida" }, 400);
        if (msg === "TRANSFER_NO_LINES")
            return c.json({ error: "Sin líneas" }, 400);
        if (msg === "TRANSFER_BAD_QTY")
            return c.json({ error: "Cantidad inválida" }, 400);
        if (msg === "TRANSFER_PRODUCT_NOT_FOUND")
            return c.json({ error: "Producto no encontrado" }, 400);
    ...
    try {
        const updated = await prisma.$transaction(async (tx) => {
            ...
            if (tr.status !== "BORRADOR")
                throw new Error("TRANSFER_BAD_STATUS");
            await assertStockForTransferSend(tx, tr.lines.map((l) => ({ productId: l.productId, qty: l.qty })));
            await applyTransferSendStock(tx, tr.lines.map((l) => ({ productId: l.productId, qty: l.qty })));
        ...
        if (msg === "TRANSFER_BAD_STATUS")
            return c.json({ error: "Solo se puede enviar un traslado en borrador" }, 400);
        if (msg === "INSUFFICIENT_STOCK")
            return c.json({ error: "Stock insuficiente para enviar" }, 400);
        if (msg === "TRANSFER_PRODUCT_NOT_FOUND")
            return c.json({ error: "Producto no encontrado" }, 400);
        throw e;
    }
    ...
    try {
        const updated = await prisma.$transaction(async (tx) => {
            ...
            if (tr.status !== "ENVIADA")
                throw new Error("TRANSFER_BAD_STATUS");
            await applyTransferReceiveStock(tx, tr.lines.map((l) => ({ productId: l.productId, qty: l.qty })));
            await tx.stockTransfer.update({
                where: { id },
...
api.post("/stock-transfers/:id/cancel", requirePermission(PERMISSION_KEYS.INVENTORY_TRANSFERS), async (c) => {
    ...
    try {
        const updated = await prisma.$transaction(async (tx) => {
            ...
            if (!tr)
                return null;
            if (tr.status === "RECIBIDA" || tr.status === "ANULADA")
                throw new Error("TRANSFER_NO_CANCEL");
            if (tr.status === "ENVIADA") {
                await applyTransferReceiveStock(tx, tr.lines.map((l) => ({ productId: l.productId, qty: l.qty })));
            }
            await tx.stockTransfer.update({
        ...
        const msg = e instanceof Error ? e.message : "";
        if (msg === "TRANSFER_NO_CANCEL")
            return c.json({ error: "No se puede anular este traslado" }, 400);
        throw e;
    }
});
...
api.post("/stock-adjustments", requireAdmin, async (c) => {
    ...
    const body = await c.req.json();
    if (!body.reason?.trim())
        return c.json({ error: "Indique el motivo del ajuste" }, 400);
    if (!body.lines?.length)
        return c.json({ error: "Agregue al menos una línea" }, 400);
    try {
        const adj = await prisma.$transaction(async (tx) => {
            for (const line of body.lines) {
                const d = Number(line.qtyDelta);
                if (!Number.isFinite(d) || d === 0)
                    throw new Error("ADJ_BAD_QTY");
                const p = await tx.product.findFirst({
                    where: { id: line.productId, organizationId: jwt.orgId },
                });
                if (!p)
                    throw new Error("PRODUCT_NOT_FOUND");
                if (p.stock + d < -0.00001)
                    throw new Error("INSUFFICIENT_STOCK");
            }
...
api.post("/employees", requireAdmin, async (c) => {
    ...
    const body = await c.req.json();
    if (!body.name?.trim())
        return c.json({ error: "Indique el nombre" }, 400);
    ...
    if (code) {
        ...
        if (dup)
            return c.json({ error: "Ya existe un empleado con ese código" }, 400);
    }
...
api.post("/expenses", requireAdmin, async (c) => {
    ...
    if (!body.category?.trim())
        return c.json({ error: "Indique la categoría" }, 400);
    const amount = Number(body.amount);
...
function resolvePayrollLineBody(L) {
    ...
    if (deductionCreates.length > 0) {
        ...
        const n = g - d;
        if (!Number.isFinite(g) || !Number.isFinite(n) || n < -0.0001)
            throw new Error("PAYROLL_BAD_NUM");
        return {
...
const payrollPeriodInclude = {
    ...
    lines: {
        include: {
            ...
            deductionItems: { orderBy: { sortOrder: "asc" } },
        },
    },
};
...
api.post("/payroll-periods", requireAdmin, async (c) => {
    ...
    if (!body.lines?.length)
        return c.json({ error: "Agregue al menos una línea de planilla" }, 400);
    try {
        ...
        const msg = e instanceof Error ? e.message : "";
        if (msg === "PAYROLL_DUP")
            return c.json({ error: "Ya existe planilla para ese mes" }, 409);
        if (msg === "EMP_NOT_FOUND")
            return c.json({ error: "Empleado no encontrado o inactivo" }, 400);
        if (msg === "PAYROLL_BAD_NUM")
            return c.json({ error: "Montos inválidos en líneas" }, 400);
        throw e;
    }
});
...
api.patch("/settings", requireAdmin, async (c) => {
    ...
    if (!s)
        s = await prisma.organizationSettings.create({ data: { organizationId: jwt.orgId } });
    const prevG = JSON.parse(s.generalJson || "{}");
    const prevI = JSON.parse(s.invoiceJson || "{}");
    const general = body.general != null ? JSON.stringify({ ...prevG, ...body.general }) : s.generalJson;
    const invoice = body.invoice != null ? JSON.stringify({ ...prevI, ...body.invoice }) : s.invoiceJson;
...
api.get("/backup/export", requireAdmin, async (c) => {
    ...
    const [org, users, products, customers, suppliers, sales, purchases, quotes, orders, settings, stockLocations, stockTransfers, stockAdjustments, employees, expenses, payrollPeriods,] = await Promise.all([

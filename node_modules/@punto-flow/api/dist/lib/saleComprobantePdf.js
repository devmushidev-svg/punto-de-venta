...
function parseInvoice(invoice) {
    ...
    if (c && typeof c === "object") {
        ...
        if (typeof o.title === "string")
            comp.title = o.title;
        if (typeof o.showSku === "boolean")
            comp.showSku = o.showSku;
    }
    return { comp, ticket };
}
...
async function loadLogoBufferForPdf(logoUrl) {
...
export async function buildSaleComprobantePdf(org, sale, invoiceJson) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (org.slogan)
            doc.text(safeLine(org.slogan), { width: leftColW });
        if (ticket.headerLine)
            doc.text(safeLine(ticket.headerLine, 400), { width: Math.min(320, leftColW) });
        if (addrLine)
            doc.text(safeLine(addrLine), { width: leftColW });
        if (org.taxId)
            doc.text(`${org.taxIdType ?? "RTN"} ${org.taxId}`);
        if (org.phone)
            doc.text(`Tel. ${safeLine(org.phone, 40)}`);
        if (org.email)
            doc.text(safeLine(org.email, 60), { width: leftColW });
        ...
        if (sale.customer) {
            ...
            if (sale.customer.taxId)
                doc.text(`ID fiscal: ${safeLine(sale.customer.taxId)}`);
            const cx = [sale.customer.address, sale.customer.phone].filter(Boolean).join(" · ");
            if (cx)
                doc.text(safeLine(cx, 200));
        }
        else {
        ...
        if (showSku)
            doc.text("Código", colSku, headerY, { width: 54 });
        ...
        for (const l of sale.lines) {
            ...
            if (showSku)
                doc.text(safeLine(l.product.sku, 12), colSku, y0, { width: 54 });
        ...
        if (sale.user?.displayName)
            doc.text(`Vendedor: ${safeLine(sale.user.displayName)}`);
        if (sale.notes)
            doc.text(`Notas: ${safeLine(sale.notes, 300)}`, { width: boxLeft - 48 });
        doc.fontSize(9);
        let ty = doc.y;
        if (showTax) {
        ...
        doc.moveDown(2);
        ensureSpace(36);
        const foot = ticket.footerLine?.trim() ||

...
import { resolveSalePaid } from "./saleTerms.js";
import { demoSeedCredentials } from "./demoSeedCredentials.js";
import {
  CUSTOMERS,
  DEMO_PURCHASES,
  DEMO_QUOTE_NOTES,
...
export async function seedDemoData(prisma: PrismaClient) {
  ...
  const ensureUser = async (username: string, displayName: string, role: string, password: string, permissionsJson?: string) => {
    const exists = await prisma.user.findFirst({
      where: { organizationId: org!.id, username },
    });
    if (exists) {
      if (permissionsJson !== undefined && exists.permissionsJson !== permissionsJson) {
        return prisma.user.update({
          where: { id: exists.id },
          data: { permissionsJson, permissionsRev: { increment: 1 } },
        });
      }
      return exists;
    }
